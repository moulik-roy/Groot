...
def summarize(): 
    ...
    java_developer_percentage = len(javaDevMatch)/len(full_stack)*100
    big_data_percentage = len(bigDataMatch)/len(Big_data)*100
    full_stack_percentage = len(fullStackMatch)/len(Java_developer)*100
    ui_developer_percentage = len(uiDeveloperMatch)/len(Ui_developer)*100

    os.remove(f.filename)
    return jsonify ({'summary': summary ,"Java_Developer": java_developer_percentage,"Bigdata_Developer":big_data_percentage,"FullStack_Developer":full_stack_percentage,"UI_Developer":ui_developer_percentage})
